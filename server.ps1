...
$inputPath = Join-Path $projectRoot "input.txt"
$outputPath = Join-Path $projectRoot "output.json"
        ...
        try {
            switch ("$($request.HttpMethod) $path") {
                ...
                "POST /api/chat" {
                    ...
                    $chatInputPath = Join-Path $projectRoot "chat_input.json"
                    $chatOutputPath = Join-Path $projectRoot "chat_output.json"
                    ...
                    try {
                        ...
                        $process = Start-Process -FilePath $pythonExe -ArgumentList "assistant_api.py" -WorkingDirectory $projectRoot -NoNewWindow -PassThru -Wait -RedirectStandardOutput $null -RedirectStandardError $null
